...
UIImage *Image::toGrayscaleUIImage() {
	// generate space for the result
	uint8_t *result=(uint8_t *) calloc(m_width*m_height,1);
	// process the image back to rgb
	for(int i=0; i<m_height*m_width; i++) {
		int val=m_imageData[i];
		result[i] = val;
	...
	// create a UIImage
	CGColorSpaceRef colorSpace=CGColorSpaceCreateDeviceGray();
	CGContextRef context=CGBitmapContextCreate(result, m_width, m_height, 8, m_width, colorSpace, kCGImageAlphaNone);
	CGImageRef image=CGBitmapContextCreateImage(context);
	CGContextRelease(context);
	CGColorSpaceRelease(colorSpace);
	UIImage *resultUIImage=[UIImage imageWithCGImage:image];
